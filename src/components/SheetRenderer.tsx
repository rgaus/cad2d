...
export const SheetRenderer: React.FunctionComponent<{
  ...
  useEffect(() => {
    ...
    return () => {
      sheet.off('defaultUnitFamilyChange', setSheetUnitFamily);
    };
  }, [sheet]);
  ...
  const draw = useCallback(
    (graphics: Graphics) => {
      ...
      const rightVisible = canvasDimensions
        ? Math.min(sheetWidth, (-vpX + canvasDimensions.width) / scale)
        : sheetWidth;
      const bottomVisible = canvasDimensions
        ? Math.min(sheetHeight, (-vpY + canvasDimensions.height) / scale)
        : sheetHeight;

      // Draw secondary grid lines (only visible ones)
      if (grid.secondarySheetUnits !== null && grid.secondaryPx !== null) {
        const secondaryWorldUnits =
          grid.secondarySheetUnits * gridToSheetFactor * SHEET_UNITS_TO_PIXELS;
