...
describe('GeometryStore', () => {
  ...
  describe('polygon web internal state', () => {
    it('creates separate webs for isolated polygons', () => {
      store.addPolygon({ points: [makePoint(0, 0)], closed: true, fillColor: null });
      store.addPolygon({ points: [makePoint(10, 10)], closed: true, fillColor: null });
      expect(store.polygonWebCount).toBe(2);
    });

    it('merges webs when polygons share a vertex', () => {
      store.addPolygon({ points: [makePoint(0, 0), makePoint(1, 0)], closed: false, fillColor: null });
      store.addPolygon({ points: [makePoint(1, 0), makePoint(2, 0)], closed: false, fillColor: null });
      expect(store.polygonWebCount).toBe(1);
      const webs = store.getAllPolygonWebs();
      expect(webs[0].polygonIds.size).toBe(2);
    });

    it('unregisterPolygonFromWeb removes polygon from web on delete', () => {
      const polygon = store.addPolygon({ points: [makePoint(0, 0), makePoint(1, 0)], closed: false, fillColor: null });
      expect(store.polygonWebCount).toBe(1);
      store.deletePolygon(polygon.id);
      expect(store.polygonWebCount).toBe(0);
    });

    it('clearAllPolygons cleans up all webs', () => {
      store.addPolygon({ points: [makePoint(0, 0)], closed: true, fillColor: null });
      store.addPolygon({ points: [makePoint(10, 10)], closed: true, fillColor: null });
      store.clearAllPolygons();
      expect(store.polygonWebCount).toBe(0);
    ...
    it('undo of deletePolygon re-registers polygon in web', () => {
      const polygon = store.addPolygon({ points: [makePoint(0, 0), makePoint(1, 0)], closed: false, fillColor: null });
      store.deletePolygon(polygon.id);
      expect(store.polygonWebCount).toBe(0);
      historyManager.undo();
    ...
    it('third polygon connecting two existing webs merges all three', () => {
      const polygonA = store.addPolygon({ 
      ...
      const polygonB = store.addPolygon({ 
        points: [makePoint(5, 0), makePoint(6, 0)], 
        closed: false, 
        fillColor: null 
      });
      expect(store.polygonWebCount).toBe(2);
      ...
      const polygonC = store.addPolygon({ 
        points: [makePoint(1, 0), makePoint(5, 0)], 
        closed: false, 
        fillColor: null 
      });
      expect(store.polygonWebCount).toBe(1);
      const webs = store.getAllPolygonWebs();
      expect(webs[0].polygonIds.size).toBe(3);
    });
  });
});
