...
describe('PolygonStore', () => {
  ...
  describe('addPolygon', () => {
    it('adds polygon to array', () => {
      const polygon: Polygon = {
        id: '1',
        points: [new SheetPosition(0, 0), new SheetPosition(10, 0), new SheetPosition(10, 10)],
        closed: true,
      };
    ...
    it('emits polygonsChanged event', () => {
      const polygon: Polygon = { id: '1', points: [new SheetPosition(0, 0)], closed: false };
      const spy = jest.fn();
  ...
  describe('updatePolygon', () => {
    it('updates existing polygon', () => {
      store.addPolygon({ id: '1', points: [new SheetPosition(0, 0)], closed: false });
  ...
  describe('deletePolygon', () => {
    it('removes polygon by id', () => {
      store.addPolygon({ id: '1', points: [new SheetPosition(0, 0)], closed: false });
      store.addPolygon({ id: '2', points: [new SheetPosition(1, 1)], closed: false });
  ...
  describe('workingPolygon', () => {
    it('setWorkingPolygon sets working polygon', () => {
      const wp = { points: [new SheetPosition(0, 0)], previewPoint: null };
      store.setWorkingPolygon(wp);
      expect(store.workingPolygon).toEqual(wp);
    });

    it('clearWorkingPolygon clears working polygon', () => {
      store.setWorkingPolygon({ points: [new SheetPosition(0, 0)], previewPoint: null });
