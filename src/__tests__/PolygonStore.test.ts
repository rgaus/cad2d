...
import type { Polygon, PointSegment } from '../lib/tools/types';
...
describe('PolygonStore', () => {
  ...
  describe('addPolygon', () => {
    ...
    it('emits polygonsChanged event', () => {
      const polygon: Polygon = { id: '1', points: [makePoint(0, 0)], closed: false };
      const spy = jest.fn();
  ...
  describe('updatePolygon', () => {
    it('updates existing polygon', () => {
      store.addPolygon({ id: '1', points: [makePoint(0, 0)], closed: false });
  ...
  describe('deletePolygon', () => {
    it('removes polygon by id', () => {
      store.addPolygon({ id: '1', points: [makePoint(0, 0)], closed: false });
      store.addPolygon({ id: '2', points: [makePoint(1, 1)], closed: false });
